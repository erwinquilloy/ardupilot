...
bool ModeAuto::_enter()
{
    ...
    // start or resume the mission based on MIS_AUTORESET, except when we
    // entered AUTO before takeoff and the mission begins with a takeoff —
    // in that case force a fresh start so the takeoff item isn't skipped
    if (plane.is_flying() || !plane.mission.starts_with_takeoff_cmd()) {
