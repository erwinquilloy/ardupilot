...
public:
    ...
    enum class Options : uint32_t {
        ...
        Use_Wh_for_remaining_percent_calc   = (1U<<23), // fork: use Wh-based remaining percent instead of mAh
    };
    ...
    AP_Float _low_cell_voltage;         /// cell voltage level used to trigger a low battery failsafe
    AP_Float _low_capacity;             /// capacity level used to trigger a low battery failsafe (mAh)
    AP_Float _low_capacity_wh;          /// capacity level used to trigger a low battery failsafe (Wh)
    AP_Float _critical_voltage;         /// voltage level used to trigger a critical battery failsafe
    AP_Float _critical_cell_voltage;    /// cell voltage level used to trigger a critical battery failsafe
    AP_Float _critical_capacity;        /// capacity level used to trigger a critical battery failsafe (mAh)
    AP_Float _critical_capacity_wh;     /// capacity level used to trigger a critical battery failsafe (Wh)
    AP_Int32 _arming_minimum_capacity;  /// capacity level required to arm
    AP_Float _arming_minimum_capacity_wh;  /// Wh capacity level required to arm
    AP_Float _arming_minimum_voltage;   /// voltage level required to arm
    AP_Int32 _options;                  /// Options
    AP_Float _cell_detect_volt;         /// cell detection voltage (full cell threshold for autodetection)
    AP_Float _cell_full_voltage;        /// minimum cell voltage to consider battery full when plugged in
    AP_Int8  _cell_count;               /// cell count override (-1 = autodetect)
