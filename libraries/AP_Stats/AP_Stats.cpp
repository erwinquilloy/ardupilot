...
const AP_Param::GroupInfo AP_Stats::var_info[] = {
    ...
    // @Description: Maximum single-flight duration recorded since last reset. Persists across reboots.
    // @Units: s
    // @ReadOnly: True
    // @User: Standard
...
void AP_Stats::set_flying(const bool status)
{
    ...
    if (status) {
        ...
        // Snapshot the just-ended flight's duration into the persistent max,
        // then reset the current-flight counter for the next one (ArduCustom
        // ca2bf6ceae). flush() compares MAX(stored, current) so the
        // persistent max is preserved even if this set_flying(false) fires
        // before the next flush tick.
        params.flight_time_max.set_and_save_ifchanged(MAX(uint32_t(params.flight_time_max), get_current_flight_time_s()));
